...
**Version 1.1 — đã resolve mâu thuẫn Trip/Settlement state machine**

**Prepared by \<Leo\>** · Rev 1.1 cập nhật bởi PM (VinaSciTech), có Claude hỗ trợ rà soát và biên tập

**\<VinaSciTech\>**

**\<date created: July 7th, 2026 — revised: July 8th, 2026\>**

> **📌 Ghi chú Rev 1.1 (08/07/2026):** Bản 1.0 có 3 chỗ mâu thuẫn nhau về Trip state machine (SF-1 vs SF-6/SF-7 vs bảng Design Constraints). Đã rà soát và **chốt dùng theo SF-1** làm chuẩn: Trip chỉ có 8 status (`draft, pendingdriver, ready, progress, end, closed, declined, canceled`), không có `await/settled/pendingclose` ở cấp Trip — các token này giờ CHỈ thuộc state machine riêng của **Settlement** (SF-6). Toàn bộ SF-6, SF-7, và bảng Design Constraints trong bản này đã được sửa lại cho khớp. Các đoạn có sửa được đánh dấu **[Rev 1.1]**. Hai vấn đề nghiệp vụ SRS gốc bỏ ngỏ (dispute settlement C-005; Trip status khi Giám đốc Reopen) vẫn cần Leo/team xác nhận chính thức — xem Appendix C.
...
| :---- | :---- | :---- | :---- |
| Leo | July 7th, 2026 | Bản khởi tạo đầy đủ SF-1 → SF-10 | 1.0 |
| PM + Claude | July 8th, 2026 | Resolve mâu thuẫn Trip/Settlement state machine giữa SF-1, SF-6, SF-7 và bảng Design Constraints; đồng bộ Document Conventions với format REQ thực tế đang dùng trong thân bài | 1.1 |
...
- **\[Rev 1.1\]** Mã yêu cầu dùng format `REQ-[MODULE]-[Number]` theo module, ví dụ `REQ-TM-001` (Trip Management), `REQ-ADV-003` (Advance Request) — đây là format thực tế được dùng xuyên suốt tài liệu này. (Bản 1.0 có ghi nhầm là `[Role]-REQ-[Number]`, ví dụ `DRV-REQ-004`, nhưng không có REQ nào trong thân bài dùng format đó — đã sửa lại mô tả cho khớp thực tế thay vì đổi hàng trăm mã REQ.)
...
| SF-9 Notification & SLA Alert | Gửi đúng thông báo đến đúng người tại đúng thời điểm trong toàn bộ vòng đời chuyến. Tự động tạo SLA Alert khi chuyến treo ở Trip `end` (Settlement chưa `settled`) quá 72h | Tất cả 4 role (nhận) | HIGH |
...
| ----- | ----- |
| Trip Status **\[Rev 1.1 — khớp SF-1 3.1.2, đây là bản chốt\]** | 8 status duy nhất: `draft` (QLVH vừa tạo chuyến) → `pendingdriver` (chờ tài xế xác nhận/từ chối) → `ready` (tài xế đã accept, chờ giải ngân) → `progress` (xe đang chạy) → `end` (tài xế đã kết thúc chuyến; TOÀN BỘ quá trình đóng chuyến 3 bên — Kế toán đối soát/quyết toán, tài xế xác nhận, QLVH đóng — diễn ra khi Trip vẫn ở `end`, không đổi status trong lúc này) → `closed` (QLVH đóng chuyến, MỘT LẦN, sau khi Settlement đã `settled` VÀ tài xế đã xác nhận). Nhánh phụ: `declined` (tài xế từ chối), `canceled` (QLVH hủy khi đang `progress`). **Không có status `pendingclose` hay `reopened` ở cấp Trip** — việc tài xế xác nhận quyết toán là 1 field (`driver_confirmed_at`) trên Trip, không phải status transition. Khi Giám đốc Reopen Settlement đã đóng, Trip vẫn giữ nguyên `closed` — chỉ Settlement đổi sang `reopened` (xem SF-6 3.6.2 và Appendix C để biết phần còn cần xác nhận thêm). |
| AdvanceRequest Status | `draft → pending → approved → disbursed`; `pending → rejected`; `approved → canceled` (chỉ trước khi `disbursed`) — xem SF-2 3.2.2 |
| Expense Status | `submitted → verified` / `need-info → resubmitted → verified\|rejected` / `rejected` — xem SF-5 3.5.2. Riêng biệt với trục `sync_status` (`Pending → Synced`) của SF-4, không phải business status |
| Settlement Status | State machine RIÊNG, độc lập với Trip: `draft → await (reviewing) → settled (confirmed) → closed`, `closed → reopened` (chỉ Giám đốc) — xem SF-6 3.6.2 |
| Trip-centric | Mọi AdvanceRequest và Settlement phải gắn trip\_id hợp lệ, status ≥ ready |
|  |  |
| Ràng buộc của Hệ thống Tài chính Doanh nghiệp | Chỉ Kế toán mới có quyền verified/rejected/need-info expense |
| Không hard-delete | Dữ liệu tài chính chỉ được void/reverse/cancel/adjust theo đúng quyền |
...
| 3-party closing | QLVH chỉ Close được (Trip: `end` → `closed`) sau khi Settlement đã `settled` VÀ tài xế đã set `driver_confirmed_at` |
| **\[Rev 1.1 — rủi ro cần audit, chưa xác nhận\]** | Nếu codebase CSS/component hiện tại đã build sẵn UI dùng token `await/settled/pendingclose` cho Trip (theo tài liệu cũ trước SRS), cần audit thực tế trước khi refactor theo model 8-status ở trên — tránh phá vỡ UI đã có mà không có kế hoạch migrate |
...
> **✅ \[Rev 1.1\] Đây là state machine CHÍNH THỨC của Trip, dùng làm chuẩn để sửa lại SF-6, SF-7 và bảng Design Constraints bên dưới — không còn mâu thuẫn nào khác trong tài liệu này.**
...
| Hành động người dùng | Response hệ thống |
| ----- | ----- |
| Trip → `end` (tài xế End) | Hệ thống tạo notification cho Kế toán: "Chuyến X cần đối soát"; đẩy vào Expense Verification Queue |
...
| REQ-VER-005 | Kế toán có màn hình Expense Verification Queue: danh sách trip đang `end`, lọc theo ngày/tài xế/tuyến; flag rõ expense có Missing Receipt | HIGH |
...
> **✅ \[Rev 1.1\] Đây là state machine của ENTITY SETTLEMENT, hoàn toàn độc lập với Trip.** Trip không có các status `await/settled/closed/reopened` này — Trip giữ nguyên `end` trong suốt quá trình Settlement chạy, chỉ chuyển `end → closed` một lần khi QLVH Close (xem SF-7).
...
| Kế toán Confirm Settlement | **\[Rev 1.1\]** Settlement: `await` → `settled` (Trip KHÔNG đổi status, vẫn giữ nguyên `end`); lock Settlement; push notification tài xế kết quả quyết toán |
...
> **⚠️ \[Rev 1.1\] Phần này ở bản 1.0 viết sai — mô tả Trip đi qua `settled → pendingclose → closed`, mâu thuẫn với SF-1. Đã sửa lại: Trip giữ nguyên `end` xuyên suốt, chỉ Settlement đổi status, và "tài xế OK" là 1 field chứ không phải Trip status.**

Trip: `end` (không đổi trong suốt luồng 3 bên) → `closed` (một lần, ở bước cuối)
Settlement: `await` → `settled` (bước 1) → `closed` (bước cuối, cùng lúc Trip đóng)
Trip.driver\_confirmed\_at: `null` → `<timestamp>` (set ở bước 2, KHÔNG phải Trip status)
...
Tài xế xem kết quả & bấm OK  
    → Trip.driver\_confirmed\_at \= now() (Trip status KHÔNG đổi, vẫn `end`)  
    → Push notification QLVH

QLVH bấm Close chuyến (chỉ enabled khi Settlement \= `settled` VÀ Trip.driver\_confirmed\_at đã có giá trị)  
    → Trip: `end` → `closed`  
...
| ----- | ----- | ----- |
| 1 | Kế toán Confirm Settlement | Settlement: `await` → `settled` (Trip vẫn giữ `end`); push notification tài xế kèm kết quả quyết toán |
...
| 3 | Tài xế bấm OK xác nhận | **\[Rev 1.1\]** Trip.`driver_confirmed_at` \= now() (Trip status KHÔNG đổi, vẫn `end`); push notification QLVH "Tài xế đã xác nhận — Chuyến X chờ đóng" |
| 4 | Tài xế bấm Từ chối / Dispute quyết toán | \[Xem Issues List C-005 — chưa có flow xử lý trong MVP\] |
| 5 | QLVH nhận notification, mở Dashboard | Nút "Close chuyến" visible và enabled vì Settlement \= `settled` VÀ Trip.`driver_confirmed_at` đã có giá trị |
| 6 | QLVH bấm Close chuyến | Trip: `end` → `closed`; Settlement: `settled` → `closed`; DriverBalance finalized |
...
| REQ-CLS-002 | **\[Rev 1.1\]** Tài xế phải bấm OK xác nhận trước khi hệ thống set `driver_confirmed_at` trên Trip; không có auto-confirm sau X giờ ở MVP; Trip status không đổi ở bước này | HIGH |
| REQ-CLS-003 | **\[Rev 1.1\]** Nút Close chuyến ở App QLVH chỉ enabled khi Settlement \= `settled` VÀ Trip.`driver_confirmed_at` đã có giá trị; disabled ở mọi trường hợp khác | HIGH |
| REQ-CLS-004 | QLVH không thể close chuyến nếu tài xế chưa bấm OK (`driver_confirmed_at` còn null) — không có override mechanism ở MVP | HIGH |
...
| REQ-CLS-006 | **\[Rev 1.1\]** Chuyến ở Trip \= `end` mà Settlement chưa `settled` quá 72h → tạo SLA Alert cho cả QLVH và Kế toán (Trip không có status `await` — xem SF-1) | HIGH |
...
| N-09 | Tài xế set `driver_confirmed_at` (bấm OK) **\[Rev 1.1, không phải Trip → pendingclose\]** | QLVH | "Tài xế đã xác nhận quyết toán — Chuyến \[mã\] chờ đóng" |
...
| Chuyến ở Trip \= `end` tròn 72h chưa có Settlement \= `settled` | Hệ thống tạo SLA Alert; gửi notification đồng thời đến QLVH và Kế toán |
...
| REQ-NTF-003 | **\[Rev 1.1\]** SLA Alert tự động tạo đúng 72h sau khi Trip chuyển sang `end` mà Settlement chưa `settled` (±5 phút); gửi đồng thời QLVH và Kế toán (Trip không có status `await` — xem SF-1) | HIGH |
...
| ----- | ----- |
| Danh sách chuyến theo status | **\[Rev 1.1\]** Filter theo: `progress` (đang chạy) / `end` chưa Settlement `settled` (chờ quyết toán) / `end` đã Settlement `settled` chưa `driver_confirmed_at` (chờ tài xế xác nhận) / `end` đã `driver_confirmed_at` (chờ QLVH đóng) / `closed` (đã đóng) — đều là sub-trạng thái hiển thị của Trip `end`, không phải Trip status riêng |
| SLA Alert panel | Danh sách chuyến treo \> 72h ở Trip `end` mà Settlement chưa `settled` |
...
| Thông tin hiển thị | Mô tả |
| ----- | ----- |
| Expense Verification Queue | Danh sách trip đang `end` cần đối soát, sắp xếp theo ngày |
...
| --- | --- | --- | --- |
| C-001 | Mâu thuẫn Trip state machine giữa SF-1 và SF-6/SF-7 | ✅ **ĐÃ RESOLVE** | Chốt dùng SF-1 (8 status, không có await/settled/pendingclose ở Trip). Xem ghi chú Rev 1.1 đầu tài liệu và mục 3.1.2 |
| C-002 | Bảng Design and Implementation Constraints ghi status Trip không khớp SF-1, và tự đặt câu hỏi bỏ ngỏ về Reopen | ✅ **ĐÃ RESOLVE** | Đã viết lại bảng cho khớp SF-1; câu hỏi Reopen → xem C-004 bên dưới |
| C-003 | Document Conventions mô tả sai format mã REQ so với thực tế dùng trong thân bài | ✅ **ĐÃ RESOLVE** | Sửa mô tả cho khớp format `REQ-[MODULE]-[Number]` thực tế đang dùng, không đổi mã REQ đã viết |
| C-004 | Giám đốc Reopen Settlement đã `closed` → Trip status xử lý ra sao | 🟡 **ĐỀ XUẤT, CHƯA CHÍNH THỨC** | Đề xuất: Trip giữ nguyên `closed` (không có Trip status `reopened`), chỉ Settlement chuyển `reopened` để Kế toán điều chỉnh. Cần Leo/PM xác nhận chính thức trước khi code |
| C-005 | Tài xế Dispute / Từ chối kết quả quyết toán | 🔴 **CHƯA CÓ FLOW** | Giữ nguyên từ bản 1.0 — chưa thiết kế cho MVP. Không tự code khi gặp, cần thiết kế riêng |
| C-006 | Ngưỡng duyệt 1.3× có nên khác nhau theo route/khách hàng B2B không | 🔴 **CHƯA QUYẾT ĐỊNH** | Theo Assumption A3 — hiện đồng nhất toàn hệ thống ở MVP |
| C-007 | Rủi ro CSS token cũ (`await/settled/pendingclose`) có thể đã tồn tại trong codebase thực tế trước khi có SRS này | 🔴 **CẦN AUDIT** | Xem bảng Design Constraints, hàng cuối. Chưa xác nhận có ảnh hưởng UI đã build hay không |