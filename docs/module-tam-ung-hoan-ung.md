> ⚠️ **ĐÃ BỊ THAY THẾ (SUPERSEDED)** — Xem `docs/SRS-advance-module.md` (v1.0, 07/07/2026) làm nguồn sự thật chính thức.
> File này giữ lại chỉ để tham khảo lịch sử. Trip state machine trong file này (`await/settled/pendingclose`) **KHÔNG còn đúng** — SRS đã đơn giản hoá thành 1 status `end` duy nhất bao trùm toàn bộ giai đoạn tài chính (xem SRS mục SF-1 3.1.2). `await/settled/closed/reopened` giờ chỉ là state machine riêng của **Settlement** (SF-6), không phải của Trip.
